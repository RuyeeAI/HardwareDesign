...
> `AgeTable` 时间戳移入独立 SRAM，`FreeList` 换成 bitmap，见 §2.1 与 §17。
...
| `FreeList.scala` | 54 | **bitmap + 优先编码器**的空闲条目池（KT 一个全局实例 + AD 一个） |
...
| `SvcEngine.scala` | 613 | **svc 共享访问引擎**：维护口 / 自学习插入 / 老化动作的状态机与决策 |
| `ForwardCam.scala` | 95 | 在途插入转发 CAM（背靠背同 KEY 先 Miss 后 Hit 的关键） |
| `ExactMatch.scala` | 915 | 顶层：存储实例 + 查找流水线 + 顶层 svc↔流水线仲裁 + 计数聚合 |
| `EmGen.scala` | 107 | Verilog 产出入口 + 6 个预设 |

子模块依赖关系：`ExactMatch` 例化 `OvfTable` / `AgeTable` / `AgeSched` / `SvcEngine` / `FreeList` / `ForwardCam`；
`OvfTable` / `AgeTable` / `AgeSched` / `SvcEngine` / `FreeList` / `ForwardCam` 都是**叶子模块**（互不例化）。
...
flowchart LR
    ...
    FL["FreeList ×2<br/>bitmap + 优先编码器<br/>(KT / AD 各一个)"]
...
| **AD** | Action Data | 1 个 `TpMemoryWrap3`；FreeList 同 KT |
...
- **引擎输出**「svc 侧」的读请求（`htRe/htRaddr`、`ktRe/ktRaddr`）与**全部写口**（HT/KT/AD 写、AgeTable ins/clr、
  OVFC wr/free、FreeList alloc/free、AgeSched release）；顶层用 `Mux(svcOwns, eng.*, 流水线)` 接进存储。
...
| FreeList | `ktOk/ktAddr/adOk/adAddr` | 分配可用性与地址 |
...
（`mtBusy` 的一部分）以及上述统计脉冲。

> 端口确实偏多（约 50 个），因为 svc 引擎与存储的**每个**端口都有耦合，无法像 `OvfTable`/`AgeSched`
> 那样自包含。换来的是 `ExactMatch` 从「1259 行、状态机与流水线交织」变成「915 行、只看流水线 + 接线」。
> 可选取舍：把 FreeList 也搬进引擎能省 8 个端口，但会破坏"引擎只管决策、不管存储分配"这条边界，故未做。

#### 2.1.4 `FreeList` —— 空闲条目池（bitmap）

`FreeList(depth)`。`bmp(i)=1` 表示条目 i 空闲；`alloc` 取**编号最低**的空闲位并清零，`free` 把 `faddr` 置 1，`count = popcount`。

- **为什么不用"寄存器栈 + sp"**：depth 大时（KT 4096 / AD 1024）栈要 `depth × addrW` 个 flop
  （4096×12 ≈ 49k flop），换成 bitmap 只要 `depth` 个 flop + 一棵优先编码树，**面积降一个数量级**。
  代价是分配顺序从 LIFO 变成"最低编号优先"——对外语义等价（任何空闲条目都可分配）。
- 同拍 `alloc + free`：净数量不变，结果 = `(位图清掉 alloc 位) | (置上 free 位)`。
- 端口：`alloc/addr/ok/free/faddr/count`。
...
|---|---|---|
| `del` | `clrEn` 顺带清 claim **并归还 KT/AD** | 老化再归还一次 → **双重释放**（FreeList 出现重复地址 → 后续 alloc 撞车 → 静默数据损坏） |
...
`dut.eng.*`（`sState` / `svcOwns` / `sUseOv` / …）、`dut.ovf.*`（`scIdx` / `use_0` / …）、
`dut.agSched.*`（`pend` / `pendOv` / …）、`dut.ageTab.*`、`dut.ktFree.*` / `dut.adFree.*`。
`em_wave.sucl` 里的路径已按这个层次更新（并逐条对照实际 VCD 的 `$scope`/`$var` 校验通过，
38/38 路径全部命中）。
...
| FreeList：`bmp → PE → 最低空闲位` | ~17 级 | **~12 级（PE 树）✅** | 已从"栈 + 4096:1 mux"换成 bitmap（§2.1.4 / §15.3） |
...
1. **`AgeTable` / `FreeList` 的面积已在本版优化掉**（原来是行为级建模的寄存器阵列 + 大 mux）：
   - `FreeList`：原来是 `stack(sp-1)` 的 4096:1 × 12bit mux（~12 级 + `sp-1` 减法共 ~17 级），
...
- 大面积 `RegInit` 已大幅收敛（`AgeTable` 的 ts 移入 SRAM、`FreeList` 换 bitmap 后，
...
9. **`SvcEngine` 端口较多**（约 50 个）：因为 svc 引擎与存储的每个端口都有耦合。若想进一步收敛，
   可把 FreeList 也搬进引擎（省 8 个端口），代价是打破"引擎只管决策、不管存储分配"这条边界。
...
| # | 现象 / 风险 | 根因 | 修法 | 详见 |
|---|---|---|---|---|
| 1 | 老化与维护删除/覆盖竞争 → **KT/AD 双重释放**（FreeList 重复地址 → 静默数据损坏） | 扫描器 claim 后、老化任务读数据前，维护路径已处理同一条目 | `S_HTD` 用 `stillClm` 复核 claim；OVFC 路径在 `S_IDLE` 启动处复核 `ovf.io.agClaim` | §8.3 |
...
| 项 | 改前 | 改后 | 收益 |
|---|---|---|---|
| `FreeList` | 寄存器栈 + `sp`：`depth × addrW` flop（KT 4096×12 ≈ 49k） | **bitmap + 优先编码器**：`depth` 个 1bit flop + PE 树 | 面积降一个数量级 |
...
结果：`ExactMatch` 1259 → **915 行**（只剩存储实例 + 查找流水线 + 顶层仲裁 + 计数聚合）；
新增 `OvfTable` 137 / `AgeSched` 84 / `SvcEngine` 613 行。
