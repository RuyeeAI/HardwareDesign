...
工具都在 `tools/em_tb/`，一条命令走完（生成 Verilog → Verilator 编译 → 跑 testbench → 起 Surfer）：

```bash
tools/em_tb/run_wave.sh              # 出 FST 并直接起 Surfer（默认）
tools/em_tb/run_wave.sh --vcd        # 出 VCD（体积大，但能用 vcd_check.py 自动检查）
tools/em_tb/run_wave.sh --no-open    # 只跑仿真，不出图形界面（CI/脚本）
tools/em_tb/run_wave.sh --no-wave    # 不出波形（Verilator 不加 --trace，仿真更快）
```

**波形开关在 Verilator 编译那一步**（不加 `--trace*` 时 TB 里的 `$dumpfile/$dumpvars` 被忽略，
TB 不用改）；**打开波形**用 `surfer [-c 预设视图] <波形文件>`，换 VaporView / GTKWave 等
也不用重新仿真 —— 详见 `tools/em_tb/README.md`。

波形与编译产物落在 `out/em_tb/`（.gitignore 内）；预设视图在 `tools/em_tb/em_wave.sucl`
（按"时钟握手 / 查找流水线 / svc 引擎 / 计数状态"分好组，已 `zoom_fit`）。
**FST 比 VCD 小约 10 倍**（同一份场景 28K vs 292K），且 Surfer/GTKWave/VaporView 都能读。

testbench 覆盖 7 个阶段并逐阶段打印观测值（没有波形查看器也能读）：复位+初始化握手 →
add（含覆盖写）→ 命中/未命中查找 → **背靠背同 KEY（需求 2）** → 连续灌命中流量（需求 1）
→ **老化并观察 KT/AD 归还（需求 3）** → **打满 HT+OVFC**。

自动检查：`run_wave.sh --vcd --no-open && python3 tools/em_tb/vcd_check.py out/em_tb/em_tb.vcd`
核对 **#(key 握手) == #(rsp_valid 高电平拍)**（见 §14.6 的来历）。

细节（`sState` 编码表、波形里怎么看各需求、Surfer 命令文件的坑）见 `tools/em_tb/README.md`。
...
- 回归：`需求2 背靠背同 KEY` 用例在收完 2 个响应后再观察 20 拍，断言**没有多余响应拍**。
  已确认这条断言有效（临时撤掉门控即失败："出现多余响应拍 1 was not equal to 0"）。
