...
仓库里原本没有任何波形；临时生成一份的完整流程（产物都在 `out/` 下，已被 .gitignore）：

```bash
sbt "runMain em.EmGen out/em tb"                 # 1) 产出 tb 预设 Verilog
cd out/em/tb
verilator --binary --timing --trace -Wno-fatal -j 4 --top-module tb \
          -o em_tb tb_exact_match.v ExactMatch.v  # 2) 编译（约 4s）
./obj_dir/em_tb                                   # 3) 跑仿真 → em_tb.vcd
```

testbench 覆盖 7 个阶段并逐阶段打印观测值（没有波形查看器也能读）：
复位+初始化握手 → add → 命中查找 → **背靠背同 KEY（需求 2）** → 连续灌命中流量（II=1）
→ **开老化并观察 KT/AD 归还（需求 3）** → **打满 HT+OVFC（需求 1 的溢出路径）**。
