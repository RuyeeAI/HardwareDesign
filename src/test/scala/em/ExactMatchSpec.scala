...
class ExactMatchSpec extends AnyFlatSpec with Matchers {
  ...
  // =========================================================================
  "需求1 每拍一请求" should "命中流量下 rsp.valid 连续不间断" in {
    val p = EmGen.preset("tb")
    val l = EmLayout(p)
    simulate(new ExactMatch(p)) { dut =>
      initDut(dut)
      ...
      var i = 0
      var cyc = 0
      while (cyc < fill) { dut.io.key.bits.poke(keys(i % keys.size).U); i += 1; dut.clock.step(1); cyc += 1 }
      ...
      dut.io.key.valid.poke(false.B)
      withClue(s"$win 拍内只收到 $rspCnt 个响应，ready 掉 $readyDrop 拍：") {
        rspCnt shouldBe win
