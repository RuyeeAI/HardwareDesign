...
class LatencyPipeSpec extends AnyFlatSpec with Matchers {

  "LatencyPipe" should "delay data by configured latency" in {
    simulate(new LatencyPipe(UInt(8.W), 3)) { c =>
  ...
  "LatencyPipe" should "pass through with latency=0" in {
    simulate(new LatencyPipe(UInt(8.W), 0)) { c =>
      c.io.in.valid.poke(true.B)
  ...
  "LatencyPipeV" should "delay valid data" in {
    simulate(new LatencyPipeV(UInt(16.W), 2)) { c =>
      c.io.in.valid.poke(true.B)
