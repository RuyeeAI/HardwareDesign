...
object RrLogic{
  def apply(rdy:UInt,point_ff:UInt):UInt={
    ...
    val double_grant: UInt = Wire(UInt((2*rdy.getWidth).W))
    double_grant := Cat(rdy,rdy) - point_ff
...
class RR (val ClientNum:Int) extends Module{
  ...
  val point_ff = RegInit(0.U(ClientNum.W))
  when (io.enable){
    point_ff := io.grant
  }
...
class WRR(val ClientNum:Int,WtWidth:Int) extends Module{
  ...
  val mask_req  = Mux(load_en,io.ready,Cat(req))
  io.grant := RR(mask_req,io.enable)
  when(load_en){
    for(i<-0 until ClientNum){
      wt(i) := io.weight(i) - io.grant(i)
    }
  }
}